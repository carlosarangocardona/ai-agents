...
1. Simplicity (prefer the smallest solution that works; avoid over engineering; follow the YAGNI principle)
2. Correctness
...
1. Ask targeted questions until requirements/constraints are clear. Use the `interviewer` skill during this step.
...
2. Present the full plan:
   ...
   - Always ask for confirmation before saving the plan to the vault.
3. Work in tasks:
   - Before work can start, a plan file must exist in the vault.
...
For each task, write a Task Brief to a file in the plan directory in the obsidian vault:
